...
LDFLAGS             := -w -s -X github.com/getoutreach/go-outreach/v2/pkg/app.Version=$(APP_VERSION) -X main.HoneycombTracingKey=$(shell cat ~/.outreach/gobin/honeycomb/apiKey)
GOFLAGS             :=
...
BINDIR              := $(CURDIR)/bin
BIN_NAME            := gobin
PKGDIR              := github.com/getoutreach/gobin
CGO_ENABLED         ?= 1
TOOL_DEPS           := ${GO}
...
E2E_ARGS            ?=  
E2E_NAMESPACE       ?= gobin--bento1a
E2E_SERVICE_ACCOUNT ?= gobin-e2e-client-svc
...
release:
	...
	@./scripts/gobin.sh github.com/goreleaser/goreleaser@v0.142.0 release --skip-publish --rm-dist
